...
/****** Object:  StoredProcedure [bdp_rpt].[sp_GVOverview_v3]    Script Date: 9/22/2026 ******/
SET ANSI_NULLS ON
GO
...
CREATE PROCEDURE [bdp_rpt].[sp_GVOverview_v3]
	@UserId INT = NULL
...
BEGIN
	...
	-- =============================================
	-- PHASE 7: OUTPUT - relabel custom-field columns (NEW in v2)
	-- Only when exactly one company was selected do the 100 custom columns get a
	-- company-specific label spliced into their alias; otherwise the paginated result
	-- set is returned as-is with no dynamic SQL at all.
	--
	-- Labels come exclusively from dbo.Dim_Company (not from any @Company_SKs/@ColumnList
	-- caller input) and are passed through QUOTENAME() before being placed in the SQL
	...
	BEGIN
		DECLARE @DebugOutput NVARCHAR(MAX) =
			'Debug Info - sp_GVOverview_v3' + CHAR(13) + CHAR(10) +
...
-- EXEC sp_helptext '[bdp_rpt].[sp_GVOverview_v3]';
-- GO
